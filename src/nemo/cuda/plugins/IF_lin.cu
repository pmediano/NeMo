#ifndef NEMO_CUDA_PLUGINS_IF_LIN_CU
#define NEMO_CUDA_PLUGINS_IF_LIN_CU
...
/*! \file IF_lin.cu Neuron update CUDA kernel for current-based
	...
	nidx_dt* s_fired)    // s_NIdx, so can handle /all/ neurons firing
{
	...
	for(unsigned bNeuron=0; bNeuron < s_partitionSize; bNeuron += THREADS_PER_BLOCK) {
		...
		if(bv_isSet(neuron, s_valid)) {
			...
			// float Ie = g_Ie0[neuron] + g_currentE[neuron];
			// float Ii = g_Ii0[neuron] + g_currentI[neuron];

			/* Update the incoming currents */
			// float I = Ie + Ii + s_currentExt[neuron] + g_I_offset[neuron];
			float I = g_currentE[neuron] + g_currentI[neuron] + s_currentExt[neuron] + g_I_offset[neuron];
